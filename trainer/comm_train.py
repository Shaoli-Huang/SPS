    ...
    for idx, (input, target) in enumerate(pbar):
        ...
        if 'baseline' not in conf.mixmethod:
            ...
            if 'inception' in conf.netname:
                ...
                loss += conf.midw*loss1

            if 'midlevel' in conf:
                if conf.midlevel:
                    loss_ma = criterion(moutput, target_a)
                    loss_mb = criterion(moutput, target_b)
                    loss += conf.midw*torch.mean(loss_ma* lam_a + loss_mb* lam_b)
        else:
            output,_,moutput = model(input)
            loss = torch.mean(criterion(output, target))

            if 'inception' in conf.netname:
                loss += conf.midw*torch.mean(criterion(moutput,target))

            if 'midlevel' in conf:
                if conf.midlevel:
                    for j in range(len(conf.sp)):
                        for i in range(conf.numswap):
                            loss += conf.midw*torch.mean(criterion(moutput[j][i],target))
