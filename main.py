...
def main(conf):
    ...
    if 'detach_epoch' in conf:
        ...
        print('detach_epoch:{}'.format(detach_epoch))
    ...
    for epoch in range(epoch_start, conf.epochs):
        ...
        if epoch > start_eval:
            with torch.no_grad():
                ...
                infostr = 'Epoch:  {:.4f}   loss: {:.4f}, H-level: {:.4f}, Best:{:.4f} ,H+M:{:.4f}'.format(epoch+1,val_loss,val_score,best_score,ascore)
                #mstr = ''
                for i in range(len(mscore)):
                    infostr += ', M-level{}:{:.4f}'.format(i,mscore[i])
                logging.info({infostr})
                save_checkpoint(
                        {'epoch': epoch + 1,
